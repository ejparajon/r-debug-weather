...
params <- list( # setting up parameters for the API query
  latitude = 40.7128, # Latitude and longitude of Johnston Country NC 
  longitude = -74.0060, 
...
# Checking data from API call (first 5 variables)
...
# This code flattens the nested jason data into an R object
...
# Check if the weather_plots.png file exists
if (!file.exists("weather_plots.png")) {
  missing <- "weather_plots.png"
} 

# Check if the historical_weather_data.rds file exists
if (!file.exists("historical_weather_data.rds")) {
  missing <- "historical_weather_data.rds"
}

# Check if both the above files are present and if so output the completion message
if (file.exists("weather_plots.png") & file.exists("historical_weather_data.rds")) {
  cat("Data download, visualization, and saving complete.\n")
} else {
  # Print missing file if required
  cat("This required file is missing:",missing, "\n")
}
